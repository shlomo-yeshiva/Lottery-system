...
<body>
    ...
    <div class="container">
        <div class="main-panel">
            ...
            <div class="upload-section">
                <h3 style="color: #ffd700; margin-bottom: 20px;">📝 הוספת שמות ידנית</h3>
                <p style="color: rgba(255,215,0,0.8); font-size: 0.95rem; margin-bottom: 12px;">הזן שמות – כל שם בשורה נפרדת, או מופרדים בפסיק</p>
                <textarea id="manualNamesInput" placeholder="ישראל כהן&#10;משה לוי&#10;רחל אברהם&#10;...&#10;&#10;או: ישראל כהן, משה לוי, רחל אברהם" rows="5" style="width: 100%; max-width: 500px; padding: 12px; border-radius: 10px; border: 2px solid rgba(255,215,0,0.3); background: rgba(0,0,0,0.3); color: #ffd700; font-size: 1rem; font-family: inherit; resize: vertical;"></textarea>
                <div style="margin-top: 12px; display: flex; gap: 10px; justify-content: center; flex-wrap: wrap;">
                    <button class="btn btn-primary" onclick="addManualNames()" style="padding: 12px 28px;">➕ הוסף שמות לרשימה</button>
                    <button class="btn btn-secondary" onclick="clearManualInput()">🗑️ נקה שדה</button>
                </div>
            </div>
            ...
            <div class="upload-section">
                ...
                <div style="margin-top: 15px;">
                    <small style="color: rgba(255,215,0,0.7);">💡 טיפ: השמות צריכים להיות בעמודה הראשונה של הקובץ. ניתן גם להוסיף שמות ידנית למעלה.</small>
                </div>
    ...
    <script>
        ...
        function addManualNames() {
            const textarea = document.getElementById('manualNamesInput');
            if (!textarea) return;
            const text = (textarea.value || '').trim();
            if (!text) {
                showNotification('הזן שמות בשדה', 'info');
                return;
            }
            // פיצול לפי שורה או פסיק
            ...
            if (newNames.length === 0) {
                showNotification('לא נמצאו שמות תקינים. הזן שמות בשורות נפרדות או מופרדים בפסיק.', 'info');
                return;
            }
            // הוספה לרשימה (ללא כפילויות)
            let added = 0;
            newNames.forEach(n => {
                if (!names.includes(n)) {
                    names.push(n);
            ...
            remainingNames = [...names];
            const startBtn = document.getElementById('startBtn');
            if (startBtn) startBtn.textContent = '🎯 התחל הגרלה';
            updateDisplay();
            showNotification(`נוספו ${added} שמות. סה״כ ברשימה: ${names.length}`, 'success');
        }

        function clearManualInput() {
            const textarea = document.getElementById('manualNamesInput');
            if (textarea) textarea.value = '';
            showNotification('השדה נוקה', 'info');
        }
